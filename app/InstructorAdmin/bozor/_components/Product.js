...
export default function Product({
  ...
  async function CreateProduct() {
    ...
    if (email1 && image) {
      ...
      try {
        if (_id) {
          await axios.put("/api/bozor", { ...data, _id });
          toast.success("Work updated!!");
        } else {
          await axios.post("/api/bozor", data);
  ...
  if (redirect) {
    router.push("/InstructorAdmin/bozor");
    return null;
  }
  console.log(image);
  function handleUploading(params) {
    params?.forEach((element) => {
      setImage(element?.url);
    });
    toast.success("uploading succesfull!!");
  }

  return (
    <div>
      <div className="sm:w-[1200px] mx-5">
        {/* Title input */}
        ...
        <div className="flex flex-col gap-2 mb-4">
          <div className="items-center my-4">
            <label className="text-lg font-medium mb-3">phone</label>
            <Input
              type="text"
              className="p-3 rounded-lg w-full"
              placeholder="+998"
        ...
        <UploadButton
          ...
          onClientUploadComplete={(res) => handleUploading(res)}
        />
        {/* Submit button */}
        <button
          type="button"
